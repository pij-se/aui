...
# aui
aui is a program library written in ANSI C that enables representation of **a**rbitrary **u**nsigned **i**ntegers, accompanied by memory management, assignment, conversion, bit manipulation, bit indexing, and mathematical operations.

'Arbitrary' is not the correct wording; there are limits to the size of an 'arbitrary' unsigned integer data structure, but it's the working title that I ended up sticking with, I hope you don't mind.

Copyright (c) 2019 Johan Palm
...
- `aui_msba()`
- `aui_lsba()`
- **TODO: Are there other functions which impose limitations?**

To avoid issues due to limitations imposed by the aforementioned functions the following restrictions must be considered:
- If the size of `AUI_TYPE` is greater than 512 bits (i.e. if `AUI_TYPE_BIT` is greater than 512) the implementation of `aui_msbn()` must be modified.
- If you need to represent values with more bits than 2 to the power of `sizeof(AUI_LEN) * CHAR_BIT` minus 1 (typically 2^32 - 1 =  4 294 967 295 bits, assuming `AUI_LEN` is defined as a 32 bit `unsigned int`), then you may redefine `AUI_LEN` to `unsigned long` (or a larger unsigned integer data type). But note that `aui_msba()` and `aui_lsba()` returns an `unsigned long` with the value `ULONG_MAX` if the arbitrary unsigned integer data structure passed to either function represents the value 0. You are thus still limited to `ULONG_MAX - 1` bits unless you modify those functions.

**TODO: Replace `unsigned int` with `AUI_LEN` definitions for `length` in `struct aui`.**

## Reliability
As stated by the license, the software is provided as is, without warranty of any kind.

The code was written to be void of --- as per the ANSI C standard --- *undefined*, *unspecified* and *implementation-defined* behaviour, and *may* thus be deterministic, assuming that the code as such, the target compiler, and the target platform are void of bugs, and that calls to memory allocation functions are guaranteed to be successful or are otherwise avoided.

Calls to memory allocation functions can be avoided by populating the internal linked list at compile-time with as many arbitrary unsigned integer data structures as are needed for the operations used. See related macros.

**TODO: Add macro to disable calls to memory allocation and freeing, and macro(s) to populate the internal linked list at compile time.** 

It is recommended to thoroughly test the code using the target compiler and the target platform.
...
**TODO: Add the deterministic-macro and linked list macro(s)**

### Functions

**TODO: Detail number and size of temporary allocs per function.**
...
- **`int aui_sets(struct aui *x, const char *string, const char *set, int base)`**
	...
	**TODO: Add description, similar to below.**
...
- **`int aui_gets(char *string, size_t size, const char *set, int base, const struct aui *x)`**
	...
	**TODO: Fix implementation. The first while-loop?**
